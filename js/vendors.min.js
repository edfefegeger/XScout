...
for (let index = 0; index < animQuest.length; index++) {
	...
	let itm = element.querySelectorAll('._itm');
	...
	for (let index = 0; index < quest.length; index++) {
		...
		element.addEventListener('click', function () {
			...
			if (element.classList.contains('_active')) {
				...
				itm[index].classList.add('_active')
				...
				itm[index].classList.remove('_active')
				setTimeout(() => {
					resp[index].style.height = `0px`
				}, 100);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    trigger.addEventListener('click', (event) => {
      ...
      customSelects.forEach(otherSelect => {
        if (otherSelect !== select) {
          otherSelect.classList.remove('custom-select--open');
        }
      });
  ...
  // Закрытие всех списков при клике вне селекта
...
setTimeout(() => {
	let slideOneSee = document.querySelector('.slide-one__see');
if(slideOneSee){
slideOneSee.addEventListener('click', function(){
	slideOneSee.closest('.slide-one__recent').classList.add('_active')
})}
let slideOneNottomSee = document.querySelectorAll('.slide-one-bottom__see');
for (let index = 0; index < slideOneNottomSee.length; index++) {
	const element = slideOneNottomSee[index];
	element.addEventListener('click', function(){
	element.closest('.slide-one-bottom__block').classList.add('_active')
})
}
}, 1000);

let appFake = document.querySelector('.app__fake');
if(appFake){
	appFake.addEventListener('click', function(){
		appFake.classList.toggle('_ready')
	})
}


let cas = document.querySelectorAll('._ca');
let caReady = document.querySelector('.ca-ready');
let home = document.querySelector('.home');
for (let index = 0; index < cas.length; index++) {
    const element = cas[index];
    element.addEventListener('click', function(){
    navigator.clipboard.writeText(element.textContent.replace('CA:',''))
    caReady.classList.add('_active')
    setTimeout(() => {
        caReady.classList.remove('_active')
    }, 1000);
    })
...
const card = document.querySelectorAll('.arc-card');
...
// Устанавливаем высоту остальным элементам
card.forEach(element => {
...
// Добавляем обработчики событий для наведения и покидания курсора
...
// Функция для обработки события движения курсора
function handleMouseMove(event) {
  ...
  const xRotation = (0.5 - (x / boundingRect.width)) * 5;
  const yRotation = (0.5 - (y / boundingRect.height)) * 20;
  const yy = y / boundingRect.height * 100
  const xx = x / boundingRect.width * 100
//   event.currentTarget.querySelector('._svett').style.transform = `translate(${xx}%, ${yy}%)`;
   event.currentTarget.querySelector('._svett').style.top = `${yy}%`;
   event.currentTarget.querySelector('._svett').style.left = `${xx}%`;
//   console.log(yy)
  // Применяем поворот к элементу
  event.currentTarget.style.transform = `rotateX(${yRotation}deg) rotateY(${xRotation}deg)`;
}
...
function handleMouseLeave(event) {
  ...
  // Возвращаем элемент в начальное состояние
